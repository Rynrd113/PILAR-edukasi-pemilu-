...
@section('content')
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-semibold text-gray-700">Edit Materi</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="md:col-span-2 space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="slug" id="slug" value="{{ old('slug', $materi->slug) }}"
                    ...
                    <div>
                        ...
                        <textarea name="konten" id="konten" rows="15" required
                                  class="w-full px-3 py-2 border @error('konten') border-red-500 @else border-gray-300 @enderror rounded-md shadow-sm focus:outline-none focus:ring-red-500 focus:border-red-500">{{ old('konten', $materi->konten) }}</textarea>
                        @error('konten')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div id="image-preview" class="mt-3 {{ $materi->gambar_path ? '' : 'hidden' }}">
                            <img id="preview-image" src="{{ $materi->gambar_path ? asset('storage/' . $materi->gambar_path) : '#' }}"
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="published_at" id="published_at"
                               value="{{ old('published_at', $materi->published_at ? $materi->published_at->format('Y-m-d\TH:i') : '') }}"
                    ...
                    <div class="border-t border-gray-200 pt-4">
                        <div class="text-sm text-gray-500 mb-4">
                            <p>Dibuat pada: {{ $materi->created_at->format('d M Y H:i') }}</p>
                            <p>Terakhir diperbarui: {{ $materi->updated_at->format('d M Y H:i') }}</p>
                            <p>Jumlah tampilan: {{ $materi->views }}</p>
                        </div>
                    </div>
...
<script>
    ...
    const easyMDE = new EasyMDE({
        ...
        autosave: {
            enabled: true,
            uniqueId: 'materi-content-{{ $materi->id }}',
            delay: 1000,
        },
        toolbar: [
            'bold', 'italic', 'heading', '|',
